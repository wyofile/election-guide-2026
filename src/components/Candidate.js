...
const Candidate = ({ slug, ballotName, status, party, color, hasPhoto, hasResponses, isIncumbent, tagId, compact = false }) => {

  const portraitPath = getPortraitPath(hasPhoto, party, slug)
  const {stories, isLoading, error} = useCachedStories(compact ? null : slug)

  if (compact) {
    return (
      <Link href={`/candidates/${slug}`} className="modern-candidate-card compact" style={{ '--party-color': color }}>
